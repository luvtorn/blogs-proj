...
class BlogService {
	async getBlogs(sortBy: string | null): Promise<Blog[]> {
		try {
			const url = sortBy ? `/blogs?sortBy=${sortBy}` : '/blogs'
			const { data } = await api.get<Blog[]>(url)
			return data
		} catch (error) {
	...
	async getBlog(id: number): Promise<BlogsResponse> {
		try {
			const { data } = await api.get<BlogsResponse>(`/blogs/blog/${id}`)
			return data
		} catch (error) {
	...
	async updateViewsCount(blog_id: number) {
		try {
			const response = await api.put(`/blogs/update-views/${blog_id}`)
			return response
		} catch (error) {
			console.error(
